...
onMount(async () => {
	...
	intervalId = setInterval(async () => {
		const response = await fetch(`${backendUrl}:8000/${projectID}/bruteForce/status`);
		...
		if (status.status === "Failed") {
			clearInterval(intervalId);
			await goto(`/project/${projectID}/tools/error?message=Internal+bruteForce+Failure`);
			...
			if(progress!==0) {
				let data = await fetch(`${backendUrl}:8000/${projectID}/bruteForce/data`);
				data = await data.json();
				requestCount += 1
				networkLinks = data.bruteForceResults;
...
<div class="page-wrapper">
	...
	<div class="table-display-area">
			<RunningResultsTableBrute networkLinks={networkLinks}/>
	</div>
		{/if}
</div>
<div class="footer-buttons">
	<div class="left-buttons">
		<ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/bruteForce/pause`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Pause</b>"></ToolButton>
		<ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/bruteForce/stop`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Stop</b>"></ToolButton>
		<ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/bruteForce/resume`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Resume</b>"></ToolButton>
		<ToolButton callback={()=>{goto(`/project/${projectID}/tools/brute-forcer`)}} content="<b>Start New Brute Forcer</b>"></ToolButton>
	</div>
