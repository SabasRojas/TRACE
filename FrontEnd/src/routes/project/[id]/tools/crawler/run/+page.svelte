<script>
    ...
    onMount(async () => {
        ...
        intervalId = setInterval(async () => {
            const response = await fetch(`${backendUrl}:8000/${projectID}/crawler/status`);
            ...
            if (status.status === "Failed") {
                ...
                await goto(`/project/${projectID}/tools/error?message=Internal+Crawler+Failure`);
                ...
                if(progress!==0) {
                    let data = await fetch(`${backendUrl}:8000/${projectID}/crawler/data`);
    ...
    function flattenNodes(nodes, targetList) {
        if (!nodes) return;
    ...
    let networkNodeList = $derived((() => {
        const list = [];
        flattenNodes(networkMap, list);
        return list;
    })());
...
<div class="page-wrapper">
    ...
    <div class="metrics">
        ...
        <div class="metric">
            ...
            <div class="metric-value">{networkNodeList.length}</div>
        </div>
        <div class="metric">
    ...
    <div class="table-display-area">
        {#key progress}
    <RunningResultsTableCrawler networkNodeList={networkNodeList}></RunningResultsTableCrawler>
            {/key}
    </div>
        {/if}
</div>
<div class="footer-buttons">
    <div class="left-buttons">
        <ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/crawler/pause`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Pause</b>"></ToolButton>
        <ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/crawler/stop`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Stop</b>"></ToolButton>
        <ToolButton callback={async ()=>{const response = await fetch(`${backendUrl}:8000/${projectID}/crawler/resume`, {method: 'POST'}); const result = await response.json(); console.log(result)}} content="<b>Resume</b>"></ToolButton>
        <ToolButton callback={()=>{goto(`/project/${projectID}/tools/crawler`)}} content="<b>Start New Crawler</b>"></ToolButton>
    </div>
